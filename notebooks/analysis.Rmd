...
brisbane_complaints_fox_plot <- brisbane_complaints %>%
 ...
 # filter(!animal_type %in% "cat" | animal_type %in% "dog") %>% 
  ...
  scale_fill_manual("Grievances", values = c("cat" = "#74c066",
                                             "dog" = "#0193cf",
                                             "other animal" = "#d6a115",
                                             "attack" = "#244321",
                                             "cat trapping" = "#73182c")
...
```
Dogs in Townsville suburbs heve been busy! Mount Luisa and Kirwan have an exceptional amount of calls regarding dogs, double of the other suburbs.
...
```{r}
euthanisation_trendcats_dogs <- national_outcomes %>%  
  filter(animal_type == "Cats" | animal_type == "Dogs") %>% 
...
```
Rehoming numbers for cats have increase drastically since the 2010s, but again I have doubts on how many of these were succesfull.
